...
{-# LANGUAGE PatternGuards, QuasiQuotes, RecordWildCards, TupleSections  #-}
...
data Opts = Opts { centering :: Bool
                 } deriving (Typeable, Data, Show)

defOpts :: Opts
defOpts = Opts { centering = def &= help "whether center the ternstile or not (default: false)"
               } &= summary "simple theorem proof assistant for LK system"
                 &= program "prover"
...
main = do
  ...
  getLine >>= proof . run sequent >>= T.putStrLn . maybe "" (render . treeToLaTeX centering) . maybeResult
